...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(pre_para):
        para = split( lower( remove_punctuation( pre_para ) ) )
        for n in topic:
            if para.count(n) > 0:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if num_typed > num_ref:
        num = num_ref
        wrong = num_typed - num_ref
        typed_words = typed_words[:num_ref]
    else:
        num = num_typed
        wrong = 0
        reference_words = reference_words[:num_typed]

    for n in list( range(num) ):
        if typed_words[n] != reference_words[n]:
            wrong += 1
    return 100*(num_typed - wrong)/num_typed
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    if valid_words.count(user_word) != 0:
        return user_word

    g = lambda n: diff_function(user_word, n, limit)
    min_diff_word = min(valid_words, key = g)
...
def shifty_shifts(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:
        ...
        if l <= limit:  #s: abc g:'' limit = 3
            return l
        else:   #s: abcde g:'' limit =3
            return limit + 1
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    time = []
    for n in times_per_player:
        time_single = []
        for i in list(range(len(n)-1)):
            time_single.append( n[i+1] - n[i] )
        time.append(time_single)
...
def fastest_words(game):
    ...
    #initialize
    fw = [ [] for i in players ]
    #fw = [[]]*len(all_times(game)) 
    #这种初始化的方法不对，[]*n指的是n个list指向同一个地址

    for i in words:
        t_min = all_times(game)[0][i]
        fastest_player = 0  #player index
        for n in players:
            if all_times(game)[n][i] < t_min:
                t_min = all_times(game)[n][i]
                fastest_player = n
        
        fw[fastest_player].append(word_at(game, i))
