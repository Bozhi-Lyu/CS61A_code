...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(root)
    if is_mobile(m):
        return tree(root, [totals_tree(end(left(m))), totals_tree(end(right(m)))])
...
def has_path(t, phrase):
    ...
    if label(t) == phrase[0]:
        ...
        FirstFlag = False
        for b in branches(t):
            FirstFlag = FirstFlag or has_path(b, phrase[1:])
        return FirstFlag
...
def quadratic(x, a, b, c):
    ...
    """
    x0 = -b/(2*a)
    f_left = a * (lower_bound(x)) * (lower_bound(x)) + b * (lower_bound(x)) + c
    f_right = a * (upper_bound(x)) * (upper_bound(x)) + b * (upper_bound(x)) + c
    f_x0 = (4*a*c - b**2)/(4*a)

    if (lower_bound(x) - x0) * (upper_bound(x) - x0) >= 0:
        up = max( f_left, f_right )
        low = min( f_left, f_right )
    else:
        up = max( f_left, f_right, f_x0 )
        low = min( f_left, f_right, f_x0 )
...
def check_par():
    ...
    r2 = interval(0.1, 1) # Replace this line!
    return r1, r2
