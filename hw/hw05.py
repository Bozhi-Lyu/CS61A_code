...
def make_withdraw(balance, password):
    ...
    def w(amount, put_in_pw):
        nonlocal balance
        nonlocal attempt_times
        
        if attempt_times >=3:
            return "Too many incorrect attempts. Attempts: ['{}', '{}', '{}']".format(wrong_pw[0], wrong_pw[1], wrong_pw[2])

        if put_in_pw != password:
            attempt_times += 1
            wrong_pw.append(put_in_pw)
...
def merge(incr_a, incr_b):
    ...
    print("DEBUG: next_a", next_a)
    print("DEBUG: next_b", next_b)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(withdraw(0, old_pass)) == str:
        ...
        pw_dic[new_pass] = old_pass
        def trans(amount, put_in_pw):
            return trans(amount, pw_dic.get(put_in_pw, put_in_pw))
        return trans
# Reference: https://zhuanlan.zhihu.com/p/507354981
