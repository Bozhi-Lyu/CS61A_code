...
class VendingMachine:
    ...
    def add_funds(self, addmoney):
        if self.Inventory == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(addmoney)
        self.balance += addmoney
        return 'Current balance: ${0}'.format(self.balance)

    def restock(self, num):
        self.Inventory += num
        return 'Current {0} stock: {1}'.format(self.prod, self.Inventory)
...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        child = t.branches[0]
        return (bst_max(child) <= t.label or bst_min(child) > t.label) and is_bst(child)
